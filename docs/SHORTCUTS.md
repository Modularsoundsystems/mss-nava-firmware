# MSS NAVA Firmware — key combinations

Every control on the panel, original and new. **Where** says which modes a
combination works in; anything not listed for a mode does nothing there.

This is the list by key combination. [MANUAL.md](MANUAL.md) explains the same
things by *what the machine does*, and has every screen drawn out.
...
Everything the original v1.028beta firmware did still works the same way.
...
| `SHIFT`+`PLAY` | arm / disarm tap recording. TAP LED blinks when armed | all |
| `TEMPO` held | show tempo; encoder sets BPM; pads show live triggers | all |
| `SHIFT`+encoder | set BPM in tenths | all but TAP |
| `TEMPO` tapped ×4 | tap tempo. Master sync only | all |
| `GUIDE` tapped | metronome on/off. A tap with `SHIFT` or `INST` held, or a hold, does not count | all |

Under external MIDI clock the tempo knob sets nothing — the rate is measured from
the clock, and shown on the bottom line with an `S` in front of it.
...
map is in [MANUAL.md §17](MANUAL.md#17-midi).
...
| `MUTE` held ≈⅓s | mute mode for as long as it is down, then back | all |
| `INST`+`PTRN` | pattern utilities — clear, move and group-paste ranges | all |
| `INST`+`NUM` | browse patterns by name, or tracks in a track mode | all |
| `SHIFT`+`TEMPO` | config pages 1 → … → 6 → out; 1–3 and 6 while running | all |
| `INST`+`GUIDE` | trig conditions, then links, then out | all |
| `SHIFT`+`ENTER` | name the pattern, or the track in a track mode | all |
...
| 2 | hi-hats — `hhP` `mte` `oh ` | yes |

| 4 | sync and MIDI — `syn` `bpm` `mTX` `mRX` (each 1–16 or `OFF`) | no |
| 5 | SysEx dump | no |
| 6 | firmware — version, commit date, commit. Read-only | yes |

The paste mask no longer has a page: hold `SHIFT`+`BANK` or `SHIFT`+`MUTE` and the
pads are the mask. `mTX`/`mRX` set to `OFF` send or receive **transport
...
| `INST`+`ENTER` | select total accent; encoder sets it, the page stays up | STEP TAP |
...
| MIDI program change | select that pattern, 0 = A01 … 127 = H16 | not TRKP/TRKW |
...
| `LASTSTEP` held+step | set pattern length; every lane goes back to following it. Held only, never latched | STEP TAP |
| `INST`+`LASTSTEP` held+step | set that instrument's own loop length. Held only | STEP TAP |
| double-tap `LASTSTEP` | nothing — `LASTSTEP` does not latch | all |
| double-tap `INST` / `SHIFT` / `CLEAR` / `SHUF` / `GUIDE` | same latch; the LED blinks while latched | all |
| double-tap `GUIDE` | …and the metronome switches off with it | all |
| `SCALE` | cycle 1/16 → 1/32 → 1/8T → 1/16T | STEP TAP |
| `SHIFT`+`SCALE` / `SHIFT`+`LASTSTEP` | +OCT / −OCT | KEYB |
| `LASTSTEP` held + encoder | nudge the pattern length | STEP TAP |
| `INST` held + encoder | walk the voices | STEP TAP |
| `BANK` held + encoder | walk the patterns, with a preview; commits on release | all |
| `TRK` held + encoder | walk the tracks, with a preview; commits on release | all |
| `SHUF` held | show shuffle and swing; steps set shuffle and flam | STEP TAP |
| `SHUF` held + encoder | the selected instrument's own swing | STEP TAP |
| `SHUF` held + `CLEAR` | put every instrument back on the pattern's shuffle | STEP TAP |
| pad held + encoder | roll rate | TAP |
| `SHIFT`+encoder | roll rate — no pad needed | TAP |
...
| `TAP` held + pad | delete that step; the later ones pull left | STEP TAP |
| `DIR` held + pad | insert a blank there; the later ones push right | STEP TAP |
| `<<` / `>>` held + pad | the same two, on panels that have them | STEP TAP |
...
| `CLEAR`+step(s) | erase those instruments as the playhead passes | TAP |
| `CLEAR`+`INST`+step(s) | wipe those whole instruments at once, on release; follows `mte` | **all** |
| `CLEAR`+`PTRN` | clear the whole pattern, running or stopped | all |
| `SHIFT`+`CLEAR` | undo, four levels. Shows `UNDO n/4` | STEP TAP PLAY MUTE |
| `INST`+`PLAY` | blocked — INST is part of nine combinations and START is next to them | all |
| `SHIFT` or `INST` **latched** | `PLAY`/`STOP` do not transport — the layer owns them | all |
| `SHIFT`+`INST`+`CLEAR` | redo. Shows `REDO n/4`. Undo and redo share the four levels | STEP TAP PLAY MUTE |
| `CLEAR`+`ENTER` | revert to the saved pattern. Shows `REVERTED` | all |
...
| `SHIFT`+`MUTE` | paste it — on release of `MUTE`, with the mask as it stands | all |
|   release `SHIFT`, keep `BANK`/`MUTE` | the mask stays open with a hand free | all |
| `SHIFT`+`BANK` or `SHIFT`+`MUTE` held + pad 1–8 | choose what a paste carries; the first eight LEDs show it | all |
| config page 4 | send a SysEx dump — encoder picks, `ENTER` held sends | stopped |
...
The eight pads are the eight groups, left to right:
...
| Keys | Does | Where |
|---|---|---|
| `SHIFT`+`PLAY` | arm recording | TAP |
| `SHIFT`+`STOP` | cyclic record on/off — screen says `CYC` | TAP |

`SHIFT`+`PLAY` and `SHIFT`+`STOP` need `SHIFT` **held**, not latched — a latch
never captures the transport.

Cyclic record is not saved: power-on is always plain record.
...
Where a pad plays or erases, the combination is the gesture itself and does not
...
| `INST` held + steps, then release | change them all together | MUTE |
| `CLEAR` held + step | solo that instrument; several can be soloed | MUTE |
| `CLEAR`+`MUTE` | solo the selected instrument, from any mode | all |
...
| pad held | roll that instrument, if the rate is not `--` | TAP |
| pad held + encoder | set the roll rate | TAP |
| `SHIFT`+encoder | set the roll rate without holding a pad | TAP |

### Backing up from the panel — config page 4
...
rather than taking a line of its own. A dash there means *follow the
...
velocity. The cap also fixes an old bug — at 1/32 the deepest shuffle put the
...
| `INST`+step | choose the instrument | |
...
## Config pages — `SHIFT`+`TEMPO`

Encoder press moves the cursor, encoder edits, `ENTER` saves.

**Page 1 — behaviour** — `pCh sTr mut cym`: pattern change timing (on the bar
line / immediately), single-trigger step edit (on/off, no weak/strong), tap
plays through a mute, and weak/strong on the crash and ride.

**Page 2 — hi-hats** — `hhP mte oh`: which hat wins when both have a trig on
the same step, whether the two mute together, and which pad names the open hat.

**Page 3 — sync and MIDI** — `syn bpm mTX mRX`: master / slave / expander, the
default tempo, and the MIDI send and receive channels (17 is `OFF`).

**Page 4 — SysEx dump.** The encoder picks what to send; `ENTER` *held* sends it.

**Page 5 — firmware.** Version, build date and build number — the number is
what to quote in a bug report.

Pages 1, 2 and 5 open while the sequencer is running. Pages 3 and 4 need it
stopped.
...
`frm` leaves `to` where it is and the range grows or shrinks against it.
...
paste mask — copy a pattern first, set the mask with the copy combination held, and
...
`INST`, `SHIFT` and `CLEAR` latch on a double-tap. `GUIDE`, `LASTSTEP` and `SHUF` do not
...
*the transport will not answer* is the worst failure this panel has.

So hold `SHIFT` for `SHIFT`+`PLAY` and `SHIFT`+`STOP`. Every other combination still
...
- **`CLEAR` still needs a real hold**, both under the playhead and with `PTRN`.
  ...
  makes acceptable. A latch may still *arm* a destructive combination —
...
mutes, pattern length, the clear combination — holding `INST` and pressing a pad picks
...
line are the state zone — up to three held or latched modifiers (`S I C F L G`,
`*` in the third cell for four or more).
...
of it; dashes there mean no clock is coming in.
...
`SHIFT` layer, and this firmware always follows it:
...
`SHUF`→swing, `GUIDE`→conditions, `PTRN`→utilities, `NUM`→browser,
...
The third is the one to remember. Undo only goes back four edits, and the copy
buffer holds whatever you last copied — after ten minutes of live playing the
edits are past counting and you no longer remember what you copied. Reloading
what is saved on the chip is the one fixed point there is.

It does nothing while a pattern change is waiting for the bar line: the incoming
pattern is about to replace everything anyway.
...
Save, undo, revert, copy and paste each take over the whole display for a
moment, and nothing can draw over them while they are up.
